(ns toshiba-global-commerce.main
  "Kotodama WASM entrypoint for the Toshiba_global_commerce clean-room actor (L4) — Clojure port.
  ...
  facts (`toshiba_global_commerce.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "toshiba_global_commerce")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "toshibag_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "toshibag_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "toshibag_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"toshiba_global_commerce.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "toshiba_global_commerce-compat" :tier tier :entities entities} 200])
...
(defn start! [] :toshiba_global_commerce-compat/ready)
